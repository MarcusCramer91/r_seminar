...
#colnames only for the original BBOB output
...
file = "C:/Users/Marcus/Desktop/monitortest/CMAES_output_1_1.txt"
data = read.table(file, skip = 1, fill = TRUE)
...
#get overall best fitness
overallBest = min(data)

#get average best
allBest = double()
for (i in min(data$run_id):max(data$run_id)) {
  allBest = c(allBest, min(data[[2]][which(data$run_id %in% i)]))
}
avgBest = mean(allBest)
